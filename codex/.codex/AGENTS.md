...
For git-backed projects, the repository's `CLAUDE.md` section `## 現在地` is the source of truth for execution status.

- Maintain exactly these fields there: `**現フェーズ**:`, `**次の一手**:`, `**懸念**:`, and `**更新**:`.
- Do not duplicate the current phase in README files.
- Obsidian project-note frontmatter fields `current_phase`, `next_action`, `concern`, and `last_touched` are generated by `~/work/projects/admin/scripts/project_mirror.py`; do not edit them manually when the project note has `local_path`.
- Projects without `local_path`, such as meeting-driven or uncloned projects, continue to be maintained directly in Obsidian.
