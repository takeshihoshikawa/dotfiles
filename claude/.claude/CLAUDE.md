...
英語の技術概念に、日本語として存在しない訳語を当てない。**書く文すべてに掛かる。**

1. **英単語に漢字1〜2字を当てはめかけたら止まる**（gate→門、floor→床、onset→発火 はこの形で生まれた）
2. 一般的な訳語・カタカナ表記があるか確認する
3. あっても読み手がその語だけで意味を取れないなら、具体語に開く（訳語の実在は合格条件ではない）
4. 無ければ英語のまま。初出だけ `gate（ゲート）` と併記し、以降はカタカナ
5. 例外——分野で定着した専門語、文書内で定義して一貫使用する語は残す（信号・較正・盲検・正本・偽陽性）

<!-- BEGIN GENERATED VOCABULARY -->
**使わない語（20）。正本は `~/dotfiles/claude/.claude/vocabulary.toml`——この一覧は生成物なので手で編集しない。**

**発火**→発生 ・ **窓**→ウィンドウ ・ **旗**→フラグ ・ **錨**→アンカー ・ **門**→ゲート ・ **床**→下限／ノイズフロア ・ **壁**→制約／限界 ・ **篩**→ふるい／フィルタ ・ **層**→レイヤ（語では切れないので検査は掛けない） ・ **粒度**→刻み／細かさ／観測単位 ・ **連担**→連結ブロック ・ **悉皆**→全数 ・ **上界**→上限 ・ **下界**→下限 ・ **看板**→中心の主張／見出し ・ **鮮度**→衛星判定率

**意味で分ける（機械置換しない）**: **帯**（band）・**掃引**（sweep）・**利得**（gain / benefit）・**製品**（(data) product）——分け方は表の備考を見る。

カタカナの長音は落とす: レイヤ・ラスタ・ベクタ・クラスタ・フォルダ・パラメータ（**ユーザーだけ長音あり**）。
迷ったらオライリー日本語版が目安。
...
**R は `renv`、Python は `uv`。システムへパッケージを直接入れない**（2026-09-15。入れる判断は解析の途中で急に来るのでここに置く）。
リポジトリに無ければ入れる前に止めて確認する。正本は同スキルの「環境管理」。
