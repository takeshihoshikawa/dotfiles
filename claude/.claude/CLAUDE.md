...
**使わない語（19）。正本は `~/dotfiles/claude/.claude/vocabulary.toml`——この一覧は生成物なので手で編集しない。**

**発火**→発生 ・ **窓**→ウィンドウ ・ **旗**→フラグ ・ **錨**→アンカー ・ **門**→ゲート ・ **床**→下限／ノイズフロア ・ **壁**→制約／限界 ・ **篩**→ふるい／フィルタ ・ **層**→レイヤ（語では切れないので検査は掛けない） ・ **粒度**→刻み／細かさ／観測単位 ・ **連担**→連結ブロック ・ **悉皆**→全数 ・ **上界**→上限 ・ **下界**→下限 ・ **看板**→中心の主張／見出し
