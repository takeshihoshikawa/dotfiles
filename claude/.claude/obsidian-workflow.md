...
- **未起動対策**: `open -a Obsidian` はアプリが起動済みでもフォアグラウンドに切り替わり画面遷移を起こすため、無条件には使わない。`pgrep -x Obsidian >/dev/null || { open -a Obsidian; sleep 2; }` のように**未起動時のみ**起動する。
...
obsidian files folder="courses"                          # ファイル一覧（sort=/limit=は非対応。CLI 1.12.7時点でオプションはfolder=/ext=/totalのみ）
...
### 更新順ノート一覧（`files`が非対応な代替）

`obsidian files` にソート・件数制限がないため、更新順に見たい場合は `find` + `stat` を使う（vaultはローカルファイルのため mtime は信頼できる）。

```bash
cd ~/vault && find . -type f -name "*.md" -not -path "./.obsidian/*" -not -path "./_assets/*" -not -path "./templates/*" \
  -exec stat -f "%m|%Sm|%N" -t "%Y-%m-%d %H:%M" {} \; | sort -rn | head -20 | cut -d'|' -f2-
```
...
タスク管理の2系統・タスク記法・追加先セクションはグローバル CLAUDE.md「## ノート・タスク管理の使い分け」が正本。
ここでは**打ち合わせ後にやること**を定義する。
...
Action Items は「ミーティングで決定した事実」の記録であり、ステータス管理はしない。転記すると
source of truth が二重化して必ず食い違うため、転記しないことがこの設計の要。

```markdown
## Action Items
- [ ] アクション内容 #project/project-name [due:: YYYY-MM-DD] [priority:: medium]
