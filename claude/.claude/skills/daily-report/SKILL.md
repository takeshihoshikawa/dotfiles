...
子項目のインデントは**4スペース**（Obsidianのタブキー既定幅と一致させる）。2スペースだと、人間が編集時にTabで階層を下げた際に既存の子項目と横位置がずれる。
...
**例外（必ず確認する）**: `~/dotfiles` の `claude/.claude/settings.json` で**差分が `model` キーのみ**の場合は、一貫したまとまりに見えても**自動コミットしない**。`/model` のピッカーは Enter でデフォルトモデルをこのファイルへ書き込む仕様（`s` がセッション限定）であり、`s` のつもりで Enter を誤爆すると意図しない変更が混入する。これをコミットすると別マシンのデフォルトを黙って書き換える（実際に過去3回、削除した `model` が別の値で復活している）。
