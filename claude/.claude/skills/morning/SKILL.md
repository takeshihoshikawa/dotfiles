...
1.5. **今週の週次計画の参照**：
   - 対象日を含む週の月曜日から7日前の日付を計算し、`weekly/weekly-{前週月曜 YYYY-MM-DD}.md` をReadツールで読み込む。週報は「振り返った週」の月曜日名で保存され、その中の `## 来週の計画` が対象日の週を指すため、参照するのは前週分になる（例: 7/22の計画は `weekly-2026-07-13.md`）
   - ファイルがあれば「## 来週の計画」内から対象日の曜日見出し（例: 対象日が7/22なら `### 水曜（7/22）`）に一致するセクションだけを抽出する。「## 来週の重点」があれば併せて拾う
   - ファイルが無い場合（前週の週次レビューをしていない）はこのステップを静かにスキップする。エラーにしない
   - 抽出できた場合のみ出力フォーマットの「📌 今週の計画」に表示する。曜日ごとの計画とタスク一覧が重複する項目があっても構わない（週次計画は文脈、タスク一覧は実行管理という役割分担）
...
## 📌 今週の計画（週次レビューがあれば表示。無ければこのセクション自体省略）

[対象日の曜日見出しの中身をそのまま。「## 来週の重点」があれば1行添える]
