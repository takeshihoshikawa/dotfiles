---
name: data-analysis-coding-conventions
description: データ分析プロジェクト（R/Python）のコーディング規約。コメントの書き方、関数化の閾値、プロジェクトをまたぐ再利用の判断、ファイル構成、スクリプト命名、パス管理、再現性、環境管理、AIエージェントとの協働時の設計・ドキュメント・テストの指針。scripts/ や src/ を書く・研究プロジェクトのファイル構成を決めるときに読む。
---
